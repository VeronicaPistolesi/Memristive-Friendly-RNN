...
param_grid_esn = {
    'num_units': [100],
    'leaky': [0.1, 0.3, 0.5],
    'input_scaling': [0.1, 0.5, 1, 1.5, 2, 5, 10],
    'bias_scaling': [0.001, 0.1, 1],
...
for dataset_name in datasets:
    ...
    n_samples, n_timesteps, n_features = x_train_all_aeon.shape
    ...
    for model_type, param_combinations, param_keys in [
                ...
                for _ in range(num_guesses):
                    if model_type == 'MF-ESN':
                        model = MFregr(
                            ...
                            memory_factor=param_dict['memory_factor'], alpha=param_dict['alpha'], gamma=param_dict['gamma'],
                            p=param_dict['p']
                        )
                    elif model_type == 'ESN':
                        model = ESNregr(
        ...
        if best_params:
            ...
            for i in range(num_guesses):
                ...
                if model_type == 'MF-ESN':
                    model = MFregr(
                        ...
                        memory_factor=best_params['memory_factor'], alpha=best_params['alpha'], gamma=best_params['gamma'],
                        p=best_params['p']
                    )
                elif model_type == 'ESN':
                    model = ESNregr(
            ...
            print(f'Results: MEAN {np.mean(rmse_ts)} STD {np.std(rmse_ts)}')
            ...
            results_logger.write(f'RMSE: MEAN {np.mean(rmse_ts)} STD {np.std(rmse_ts)}\n')
            ...
            trainable_parameters = model.readout.coef_.size + model.readout.intercept_.size
            results_logger.write(f'Trainable parameters = {trainable_parameters}\n')
